...
import { type Profile, hasCouselorPermissions, hasManagerPermissions } from "./Web3Service";
...
export async function addApiResident(resident: ApiResident): Promise<ApiResident> {
    if(!hasCouselorPermissions()) throw new Error(`You do not have permissions`);
